...
def cleanup_intermediate_files(index_name):
    ...
    safe_remove(Z_ORDER_OUTPUT_3D)
    safe_remove(RANK_SPACE_Z_ORDER_OUTPUT_3D)
    safe_remove(BMTREE_INPUT)
    # safe_remove(CHOOSE_SUBTREE_MODEL_NAME_3D)
    # safe_remove(SPLIT_MODEL_NAME_3D)
...
def execute_range_query(data_file, query_file, range_query_output_path, test_file="test-rtree-RTreeQuery3d", index_name="tree"):
...
def execute_spatial_join(data_file, query_file, spatial_join_output_path, test_file="test-rtree-RTreeQuery3d", index_name="tree"):
...
def execute_knn_query(k, query_file, data_file, knn_query_output_path, test_file="test-rtree-RTreeQuery3d", index_name="tree"):
...
def execute_point_query(query_file, data_file, point_query_output_path, test_file="test-rtree-RTreeQuery3d", index_name="tree"):
...
def execute_insert(query_file, insert_output_path, test_file="test-rtree-RTreeQuery3d", index_name="tree"):
...
def execute_insert_point(query_file, insert_point_output_path, test_file="test-rtree-RTreeQuery3d", index_name="tree"):
...
def run_zorder(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        z_order_output_default = Z_ORDER_SORTED_DEFAULT_3D.format(
            data_file_prefix=data_file_prefix,
            bit_num=bit_num
        )

        data_file = Z_ORDER_SORTED_OUTPUT_3D

        if not os.path.exists(z_order_output_default):
            logger.info(f"{z_order_output_default} NOT exists")

            transform_command = f"python tools/zorder_3d.py {ablosute_data_file_name} {Z_ORDER_OUTPUT_3D} {bit_num}"
            elapsed_time_ns_order = execute_command(transform_command)

            format_data_command = f"python tools/libspatialindex_data_adapter.py --type data --dimensions 3 --input {Z_ORDER_OUTPUT_3D} --output {data_file}"
        ...
        command = f"test-rtree-SFCRTreeBulkLoad3d {data_file} {INDEX_PATH}/zorder {page_size} {fill_factor} {BLOCK_SIZE} {BUFFER}"
        ...
        if point_queries or knn_queries:
            build_output_path = Z_BUILD_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                bit_num=bit_num
            )
        ...
        for file_name in range_queries:
            ...
            range_query_output_path = Z_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                bit_num=bit_num,
            )
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            execute_range_query(data_file, query_file, range_query_output_path, "test-rtree-RTreeQuery3d", index_name="zorder")

        for file_name in spatial_joins:
            file_name_prefix = file_name.rstrip('.csv')
            join_output_path = Z_JOIN_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                bit_num=bit_num,
            )
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            execute_spatial_join(data_file, query_file, join_output_path, "test-rtree-RTreeQuery3d", index_name="zorder")
        ...
        for file_name in knn_queries:
            ...
            for k in ks:
                knn_query_output_path = Z_KNN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    knn_query_prefix=file_name_prefix,
                    bit_num=bit_num,
                    k=k
                )
                execute_knn_query(k, query_file, data_file, knn_query_output_path, "test-rtree-RTreeQuery3d", index_name="zorder")

        for file_name in point_queries:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            point_query_output_path = Z_POINT_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                point_query_prefix=file_name_prefix,
                bit_num=bit_num,
            )
            execute_point_query(query_file, data_file, point_query_output_path, "test-rtree-RTreeQuery3d", index_name="zorder")

        for file_name in insertions:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_output_path = Z_INSERT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_prefix=file_name_prefix,
                bit_num=bit_num,
            )
            execute_insert(query_file, insert_output_path, "test-rtree-RTreeQuery3d", index_name="zorder")

        for file_name in insert_points:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_point_output_path = Z_INSERT_POINT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_point_prefix=file_name_prefix,
                bit_num=bit_num,
            )
            execute_insert_point(query_file, insert_point_output_path, "test-rtree-RTreeQuery3d", index_name="zorder")
        ...
        safe_remove(Z_ORDER_SORTED_OUTPUT_3D)
...
def run_zm(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        data_file = RANK_SPACE_Z_ORDER_SORTED_OUTPUT_3D
        ...
        if not os.path.exists(z_order_output_default):
            ...
            logger.info(f"{z_order_output_default} NOT exists")
            transform_command = f"python tools/rank_space_z.py {ablosute_data_file_name} {RANK_SPACE_Z_ORDER_OUTPUT_3D} {bit_num}"
            elapsed_time_ns_order = execute_command(transform_command)
            ...
            format_data_command = f"python tools/libspatialindex_data_adapter.py --type data --dimensions 3 --input {RANK_SPACE_Z_ORDER_OUTPUT_3D} --output {data_file} --is_learned True"
        ...
        build_output_path = ZM_BUILD_OUTPUT_PATH_3D.format(
                        data_file_prefix=data_file_prefix,
                        bit_num=bit_num,
                    )

        command = f"test-learnedindex-ZMBulkLoad3d {data_file} {INDEX_PATH}/zm {page_size} {fill_factor} {BLOCK_SIZE} {BUFFER}"
        result, elapsed_time_ns_build = execute_command_with_err(command)
        ...
        for range_file_name in range_queries:
            ...
            range_query_output_path = ZM_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                bit_num=bit_num,
            )
            execute_range_query(data_file, query_file, range_query_output_path, test_file="test-learnedindex-ZMQuery3d", index_name="zm")

        for join_file_name in spatial_joins:
            file_name_prefix = join_file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix + "_zm")
            join_query_output_path = ZM_JOIN_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                bit_num=bit_num,
            )
            execute_spatial_join(data_file, query_file, join_query_output_path, test_file="test-learnedindex-ZMQuery3d", index_name="zm")
        ...
        for knn_file_name in knn_queries:
            ...
            for k in ks:
                knn_query_output_path = ZM_KNN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    knn_query_prefix=knn_file_name_prefix,
                    k=k,
                    bit_num=bit_num,
                )
                execute_knn_query(k, knn_query_file, data_file, knn_query_output_path, test_file="test-learnedindex-ZMQuery3d", index_name="zm")

        for file_name in point_queries:
            point_file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, point_file_name_prefix + "_zm")
            point_query_output_path = ZM_POINT_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                point_query_prefix=point_file_name_prefix,
                bit_num=bit_num,
            )
            execute_point_query(query_file, data_file, point_query_output_path, test_file="test-learnedindex-ZMQuery3d", index_name="zm")

        for file_name in insertions:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_output_path = ZM_PARTITION_OUTPUT_INSERT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_prefix=file_name_prefix,
                bit_num=bit_num
            )
            execute_insert(query_file, insert_output_path, test_file="test-learnedindex-ZMQuery3d", index_name="zm")

        for file_name in insert_points:
            insert_point_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_point_output_path = ZM_PARTITION_OUTPUT_INSERT_POINT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_point_prefix=insert_point_prefix,
                bit_num=bit_num
            )
            execute_insert_point(query_file, insert_point_output_path, test_file="test-learnedindex-ZMQuery3d", index_name="zm")
        ...
        safe_remove(RANK_SPACE_Z_ORDER_SORTED_OUTPUT_3D)
        safe_remove(z_order_output_default)
...
def run_lisa(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        data_file = LISA_DATA_3D
        format_data_command = f"python tools/libspatialindex_data_adapter.py --type data --dimensions 3 --input {ablosute_data_file_name} --output {data_file}"
        execute_command(format_data_command)

        logger.info(f"Start building lisa: {data_file}")

        build_output_path = LISA_BUILD_OUTPUT_PATH_3D.format(
                        data_file_prefix=data_file_prefix
                    )

        command = f"test-learnedindex-LISABulkLoad3d {data_file} {INDEX_PATH}/lisa {page_size} {fill_factor} {BLOCK_SIZE} {BUFFER}"
        result, elapsed_time_ns_build = execute_command_with_err(command)
        ...
        for range_file_name in range_queries:
            file_name_prefix = range_file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            range_query_output_path = LISA_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix
            )
            execute_range_query(data_file, query_file, range_query_output_path, test_file="test-learnedindex-LISAQuery3d", index_name="lisa")

        for join_file_name in spatial_joins:
            file_name_prefix = join_file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            join_query_output_path = LISA_JOIN_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix
            )
            execute_spatial_join(data_file, query_file, join_query_output_path, test_file="test-learnedindex-LISAQuery3d", index_name="lisa")
        ...
        for knn_file_name in knn_queries:
            ...
            ks = ks_map.get(knn_file_name)
            for k in ks:
                knn_query_output_path = LISA_KNN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    knn_query_prefix=knn_file_name_prefix,
                    k=k,
                )
                execute_knn_query(k, knn_query_file, data_file, knn_query_output_path, test_file="test-learnedindex-LISAQuery3d", index_name="lisa")

        for file_name in point_queries:
            point_file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, point_file_name_prefix)
            point_query_output_path = LISA_POINT_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                point_query_prefix=point_file_name_prefix
            )
            execute_point_query(query_file, data_file, point_query_output_path, test_file="test-learnedindex-LISAQuery3d", index_name="lisa")

        for file_name in insertions:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_output_path = LISA_INSERT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_prefix=file_name_prefix
            )
            execute_insert(query_file, insert_output_path, test_file="test-learnedindex-LISAQuery3d", index_name="lisa")

        for file_name in insert_points:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_point_output_path = LISA_INSERT_POINT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_point_prefix=file_name_prefix
            )
            execute_insert_point(query_file, insert_point_output_path, test_file="test-learnedindex-LISAQuery3d", index_name="lisa")
...
def run_rankspace(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        rank_space_z_order_output_default = RANK_SPACE_Z_ORDER_SORTED_DEFAULT_3D.format(
            data_file_prefix=data_file_prefix,
            bit_num=bit_num
        )
        ...
        if not os.path.exists(rank_space_z_order_output_default):
            ...
            format_data_command = f"python tools/libspatialindex_data_adapter.py --type data --dimensions 3 --input {RANK_SPACE_Z_ORDER_OUTPUT_3D} --output {data_file}"
            execute_command(format_data_command)
        ...
        command = f"test-rtree-SFCRTreeBulkLoad3d {data_file} {INDEX_PATH}/rankspace {page_size} {fill_factor} {BLOCK_SIZE} {BUFFER}"

        result, elapsed_time_ns_build = execute_command_with_err(command)

        build_output_path = RANK_SPACE_Z_BUILD_OUTPUT_PATH_3D.format(
            data_file_prefix=data_file_prefix,
            bit_num=bit_num,
        )
        ...
        for file_name in range_queries:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            range_query_output_path = RANK_SPACE_Z_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                bit_num=bit_num,
            )
        ...
        for file_name in spatial_joins:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            join_query_output_path = RANK_SPACE_Z_JOIN_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                bit_num=bit_num,
            )
        ...
        for file_name in knn_queries:
            ...
            ks = ks_map.get(file_name)
            for k in ks:
                knn_query_output_path = RANK_SPACE_Z_KNN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    knn_query_prefix=file_name_prefix,
                    bit_num=bit_num,
                    k=k
        ...
        for file_name in point_queries:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            point_query_output_path = RANK_SPACE_Z_POINT_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                point_query_prefix=file_name_prefix,
                bit_num=bit_num,
            )
        ...
        for file_name in insertions:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_output_path = RANK_SPACE_Z_INSERT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_prefix=file_name_prefix,
                bit_num=bit_num,
            )
        ...
        for file_name in insert_points:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_point_output_path = RANK_SPACE_Z_INSERT_POINT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_point_prefix=file_name_prefix,
                bit_num=bit_num,
            )
...
def run_bmtree(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        for range_file_name in range_queries:
            ...
            bmtree_model_output_default = BMTREE_MODEL_OUTPUT_3D.format(
                data_file_prefix=data_file_prefix,
            ...
            if is_train:
                ...
                try:
                    shutil.copy(bmtree_model_output_default, BMTREE_MODEL_OUTPUT_DEFAULT_3D)
                    logger.info(f"Copied {bmtree_model_output_default} to {BMTREE_MODEL_OUTPUT_DEFAULT_3D}")
                except Exception as e:
                    logger.error(f"Failed to copy {bmtree_model_output_default} to {BMTREE_MODEL_OUTPUT_DEFAULT_3D}: {e}")
            ...
            learn_bmtree_command = f"bash rl_baseline/learn_bmtree.sh {data_file_prefix} {file_name_prefix} {tree_depth} {sample_size} {bit_num} {ablosute_data_file_name} {is_train} {0} 3"
            elapsed_time_ns_learn = execute_command(learn_bmtree_command)

            data_adapter_command = f"python tools/libspatialindex_data_adapter.py --type data --dimensions 3 --is_scaled --input {BMTREE_INPUT} --output {BMTREE_OUTPUT_3D}"
            ...
            command = f"test-rtree-SFCRTreeBulkLoad3d {BMTREE_OUTPUT_3D} {INDEX_PATH}/bmtree {page_size} {fill_factor} {BLOCK_SIZE} {BUFFER}"
            result, elapsed_time_ns_build = execute_command_with_err(command)

            if point_queries or knn_queries:
                build_output_path = BMTREE_BUILD_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    query=file_name_prefix,
                    bit_num=bit_num,
                    tree_depth=tree_depth,
            ...
            data_file = BMTREE_OUTPUT_3D

            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            range_query_output_path = BMTREE_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                bit_num=bit_num,
                tree_depth=tree_depth,
            ...
            for knn_file_name in knn_queries:
                ...
                ks = ks_map.get(knn_file_name)
                for k in ks:
                    knn_query_output_path = BMTREE_KNN_QUERY_OUTPUT_PATH_3D.format(
                        data_file_prefix=data_file_prefix,
                        range_query_prefix=file_name_prefix,
                        bit_num=bit_num,
                        tree_depth=tree_depth,
            ...
            for spatial_join_file_name in spatial_joins:
                ...
                join_output_path = BMTREE_JOIN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
            ...
            for file_name in point_queries:
                point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, point_file_name_prefix)
                point_query_output_path = BMTREE_POINT_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    point_query_prefix=point_file_name_prefix,
                    bit_num=bit_num,
            ...
            for file_name in insertions:
                ...
                insert_output_path = BMTREE_INSERT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
            ...
            for file_name in insert_points:
                ...
                insert_point_output_path = BMTREE_INSERT_POINT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
...
def run_bmtree_impr(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        for range_file_name in range_queries:
            ...
            bmtree_model_output_default = BMTREEIMPR_MODEL_OUTPUT_3D.format(
                data_file_prefix=data_file_prefix,
                query=file_name_prefix,
                bit_num=bit_num,
                tree_depth=tree_depth,
            ...
            if is_train:
                ...
                # Try copying the file to the specified destination
                try:
                    shutil.copy(bmtree_model_output_default, BMTREEIMPR_MODEL_OUTPUT_DEFAULT_3D)
                    logger.info(f"Copied {bmtree_model_output_default} to {BMTREEIMPR_MODEL_OUTPUT_DEFAULT_3D}")
                except Exception as e:
                    logger.error(f"Failed to copy {bmtree_model_output_default} to {BMTREEIMPR_MODEL_OUTPUT_DEFAULT_3D}: {e}")
                
            data_transfer_command = f"python rl_baseline/bmtree_data_transfer.py {ablosute_data_file_name} {ablosute_query_file_name}"
            execute_command(data_transfer_command)

            cost_method = 1
            learn_bmtree_command = f"bash rl_baseline/learn_bmtree.sh {data_file_prefix} {file_name_prefix} {tree_depth} {sample_size} {bit_num} {ablosute_data_file_name} {is_train} {cost_method} 3"
            elapsed_time_ns_learn = execute_command(learn_bmtree_command)

            data_adapter_command = f"python tools/libspatialindex_data_adapter.py --type data --dimensions 3 --is_scaled --input {BMTREEIMPR_INPUT} --output {BMTREEIMPR_OUTPUT_3D}"
            execute_command(data_adapter_command)
            ...
            # build bmtree sfcrtree
            command = f"test-rtree-SFCRTreeBulkLoad3d {BMTREEIMPR_OUTPUT_3D} {INDEX_PATH}/bmtree_impr {page_size} {fill_factor} {BLOCK_SIZE} {BUFFER}"
            result, elapsed_time_ns_build = execute_command_with_err(command)

            if point_queries or knn_queries:
                build_output_path = BMTREEIMPR_BUILD_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    query=file_name_prefix,
                    bit_num=bit_num,
                    tree_depth=tree_depth,
            ...
            data_file = BMTREEIMPR_OUTPUT_3D

            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            range_query_output_path = BMTREEIMPR_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                bit_num=bit_num,
                tree_depth=tree_depth,
            ...
            for knn_file_name in knn_queries:
                ...
                ks = ks_map.get(knn_file_name)
                for k in ks:
                    knn_query_output_path = BMTREEIMPR_KNN_QUERY_OUTPUT_PATH_3D.format(
                        data_file_prefix=data_file_prefix,
                        range_query_prefix=file_name_prefix,
                        bit_num=bit_num,
                        tree_depth=tree_depth,
            ...
            for spatial_join_file_name in spatial_joins:
                spatial_join_file_name_prefix = spatial_join_file_name.rstrip('.csv')
                join_output_path = BMTREEIMPR_JOIN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
            ...
            for file_name in point_queries:
                point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, point_file_name_prefix)
                point_query_output_path = BMTREEIMPR_POINT_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    point_query_prefix=point_file_name_prefix,
                    bit_num=bit_num,
            ...
            for file_name in insertions:
                insert_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, insert_file_name_prefix)
                insert_output_path = BMTREEIMPR_INSERT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    insert_prefix=insert_file_name_prefix,
                    bit_num=bit_num,
            ...
            for file_name in insert_points:
                insert_point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, insert_point_file_name_prefix)
                insert_point_output_path = BMTREEIMPR_INSERT_POINT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    insert_point_prefix=insert_point_file_name_prefix,
                    bit_num=bit_num,
        ...
        safe_remove(BMTREEIMPR_OUTPUT_3D)
...
def run_rtree(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        command = f"test-rtree-RTreeLoad3d {data_file} {INDEX_PATH}/rtree {page_size} {fill_factor} {rtree_variant} {BLOCK_SIZE} {BUFFER}"

        result, elapsed_time_ns_build = execute_command_with_err(command)

        if point_queries or knn_queries:
            build_output_path = RTREE_BUILD_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
        ...
        for file_name in range_queries:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            range_query_output_path = RTREE_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                variant=rtree_variant
            )
        ...
        for file_name in spatial_joins:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            join_query_output_path = RTREE_JOIN_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                variant=rtree_variant
            )
        ...
        for file_name in knn_queries:
            ...
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            for k in ks:
                knn_query_output_path = RTREE_KNN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    knn_query_prefix=file_name_prefix,
                    k=k,
                    variant=rtree_variant
        ...
        for file_name in point_queries:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            point_query_output_path = RTREE_POINT_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                point_query_prefix=file_name_prefix,
                variant=rtree_variant
            )
        ...
        for file_name in insertions:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_output_path = RTREE_INSERT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_prefix=file_name_prefix,
                variant=rtree_variant
            )
        ...
        for file_name in insert_points:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_point_output_path = RTREE_INSERT_POINT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_point_prefix=file_name_prefix,
                variant=rtree_variant
            )
...
def run_rstartree(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        command = f"test-rtree-RTreeLoad3d {data_file} {INDEX_PATH}/rstar {page_size} {fill_factor} {rtree_variant} {BLOCK_SIZE} {BUFFER}"

        result, elapsed_time_ns_build = execute_command_with_err(command)

        if point_queries or knn_queries:
            build_output_path = R_STAR_TREE_BUILD_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                variant=rtree_variant
            )
        ...
        for file_name in range_queries:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            range_query_output_path = R_STAR_TREE_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                variant=rtree_variant
            )
        ...
        for file_name in spatial_joins:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            join_query_output_path = R_STAR_TREE_JOIN_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                variant=rtree_variant
            )
        ...
        for file_name in knn_queries:
            ...
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            for k in ks:
                knn_query_output_path = R_STAR_TREE_KNN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    knn_query_prefix=file_name_prefix,
                    k=k,
                    variant=rtree_variant
        ...
        for file_name in point_queries:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            point_query_output_path = R_STAR_TREE_POINT_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                point_query_prefix=file_name_prefix,
                variant=rtree_variant
            )
        ...
        for file_name in insertions:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_output_path = R_STAR_TREE_INSERT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_prefix=file_name_prefix,
                variant=rtree_variant
            )
        ...
        for file_name in insert_points:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_point_output_path = R_STAR_TREE_INSERT_POINT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_point_prefix=file_name_prefix,
                variant=rtree_variant
            )
        ...
        safe_remove(R_STAR_TREE_DATA_3D)
...
def run_rlrtree(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        model_path = baseline_config.get("model_path", RLRTREE_MODEL_PATH_3D)

        data_file = RLRTREE_DATA_3D
        ...
        for range_file_name in range_queries:
            ...
            split_model_name_default = SPLIT_MODEL_NAME_DEFAULT_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                variant=rtree_variant,
                epoch=epoch,
            ...
            choose_subtree_model_name_default = CHOOSE_SUBTREE_MODEL_NAME_DEFAULT_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                variant=rtree_variant,
                epoch=epoch,
            ...
            command = f"test-rtree-RTreeLoad3d {data_file} {INDEX_PATH}/rlrtree {page_size} {fill_factor} {rtree_variant} {model_path} {BLOCK_SIZE} {BUFFER}"
            result, elapsed_time_ns_build = execute_command_with_err(command)

            if point_queries or knn_queries:
                build_output_path = RLRTREE_BUILD_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    variant=rtree_variant,
                    epoch=epoch,
            ...
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            range_query_output_path = RLRTREE_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                variant=rtree_variant,
                epoch=epoch,
            ...
            for file_name in point_queries:
                point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, point_file_name_prefix)
                point_query_output_path = RLRTREE_POINT_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    point_query_prefix=point_file_name_prefix,
                    epoch=epoch,
            ...
            for file_name in spatial_joins:
                ...
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, point_file_name_prefix)
                join_query_output_path = RLRTREE_JOIN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                ...
                execute_spatial_join(query_file, data_file, join_query_output_path, index_name="rlrtree")
            ...
            for file_name in knn_queries:
                ...
                knn_query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, knn_file_name_prefix)
                for k in ks:
                    knn_query_output_path = RLRTREE_KNN_QUERY_OUTPUT_PATH_3D.format(
                        data_file_prefix=data_file_prefix,
                        range_query_prefix=file_name_prefix,
                        knn_query_prefix=knn_file_name_prefix,
                        epoch=epoch,
            ...
            for file_name in insertions:
                insert_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, insert_file_name_prefix)
                insert_output_path = RLRTREE_INSERT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    insert_prefix=insert_file_name_prefix,
                    epoch=epoch,
            ...
            for file_name in insert_points:
                insert_point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, insert_point_file_name_prefix)
                insert_point_output_path = RLRTREE_INSERT_POINT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    insert_point_prefix=insert_point_file_name_prefix,
                    epoch=epoch,
...
def run_kdtree(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        command = f"test-kdtree-KDTreeBulkLoad3d kdtree {data_file} path {INDEX_PATH}/kdtree {page_size} 1.0 {BLOCK_SIZE} {BUFFER}"  
        ...
        if point_queries or knn_queries:
            build_output_path = KDTREE_BUILD_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix
            )
        ...
        for file_name in range_queries:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            range_query_output_path = KDTREE_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix
            )
            execute_range_query(data_file, query_file, range_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdtree")

        for file_name in spatial_joins:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            join_query_output_path = KDTREE_JOIN_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix
            )
            execute_spatial_join(data_file, query_file, join_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdtree")
        ...
        for file_name in knn_queries:
            ...
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            for k in ks:
                knn_query_output_path = KDTREE_KNN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    knn_query_prefix=file_name_prefix,
                    k=k
                )
                execute_knn_query(k, query_file, data_file, knn_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdtree")

        for file_name in point_queries:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            point_query_output_path = KDTREE_POINT_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                point_query_prefix=file_name_prefix,
            )
            execute_point_query(query_file, data_file, point_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdtree")

        for file_name in insertions:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_output_path = KDTREE_INSERT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_prefix=file_name_prefix,
            )
            execute_insert(query_file, insert_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdtree")

        for file_name in insert_points:
            file_name_prefix = file_name.rstrip('.csv')
            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            insert_point_output_path = KDTREE_INSERT_POINT_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                insert_point_prefix=file_name_prefix,
            )
            execute_insert_point(query_file, insert_point_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdtree")
...
def run_kdtree_greedy(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        page_size = baseline_config.get("page_size", 100)
        data_file = KDTREE_GREEDY_DATA_3D
        ...
        for range_file_name in range_queries:
            ...
            command = f"test-kdtree-KDTreeBulkLoad3d greedy_kdtree {data_file} {query_file} {INDEX_PATH}/kdgreedy {page_size} 1.0 {BLOCK_SIZE} {BUFFER}"  
            ...
            if point_queries or knn_queries:
                build_output_path = KDTREE_GREEDY_BUILD_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix
                )
            ...
            range_query_output_path = KDTREE_GREEDY_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix
            )
            execute_range_query(data_file, query_file, range_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdgreedy")
            ...
            for file_name in knn_queries:
                ...
                for k in ks:
                    knn_query_output_path = KDTREE_GREEDY_KNN_QUERY_OUTPUT_PATH_3D.format(
                        data_file_prefix=data_file_prefix,
                        range_query_prefix=file_name_prefix,
                        knn_query_prefix=knn_file_name_prefix,
                        k=k
                    )
                    execute_knn_query(k, query_file, data_file, knn_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdgreedy")

            for file_name in spatial_joins:
                join_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, join_file_name_prefix)
                join_query_output_path = KDTREE_GREEDY_JOIN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    join_query_prefix=join_file_name_prefix,
                )
                execute_spatial_join(query_file, data_file, join_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdgreedy")

            for file_name in point_queries:
                point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, point_file_name_prefix)
                point_query_output_path = KDTREE_GREEDY_POINT_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    point_query_prefix=point_file_name_prefix,
                )
                execute_point_query(query_file, data_file, point_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdgreedy")

            for file_name in insertions:
                insert_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, insert_file_name_prefix)
                insert_output_path = KDTREE_GREEDY_INSERT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    insert_prefix=insert_file_name_prefix,
                )
                execute_insert(query_file, insert_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdgreedy")

            for file_name in insert_points:
                insert_point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, insert_point_file_name_prefix)
                insert_point_output_path = KDTREE_GREEDY_INSERT_POINT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    insert_point_prefix=insert_point_file_name_prefix,
                )
                execute_insert_point(query_file, insert_point_output_path, "test-kdtree-KDTreeQuery3d", index_name="kdgreedy")
        ...
        safe_remove(KDTREE_GREEDY_DATA_3D)
...
def run_qdtree_rl(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        model_path = baseline_config.get("model_path", QDTREE_MODEL_PATH_3D)
        action_sampling_size = baseline_config.get("action_sampling_size", None)
        data_file = QDTREE_DATA_3D
        ...
        for range_file_name in range_queries:
            ...
            qdtree_model_name_default = QDTREE_MODEL_NAME_DEFAULT_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                episode=episode,
            ...
            if not os.path.exists(qdtree_model_name_default):
                ...
                learn_qdtree_command = f"bash rl_baseline/learn_qdtree.sh {ablosute_data_file_name} {ablosute_query_file_name} {episode} {sampling_ratio} {action_sampling_size} 3"
            ...
            command = f"test-kdtree-QDTreeBulkLoad3d qdtree {data_file} {query_file} {INDEX_PATH}/qdtree {page_size} 1.0 {model_path} {action_sampling_size} {BLOCK_SIZE} {BUFFER}"  
            ...
            if point_queries or knn_queries:
                build_output_path = QDTREE_BUILD_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    episode=episode,
                    sampling_ratio=sampling_ratio,
            ...
            range_query_output_path = QDTREE_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
                episode=episode,
                sampling_ratio=sampling_ratio,
            ...
            execute_range_query(data_file, query_file, range_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="qdtree")
            ...
            for file_name in knn_queries:
                ...
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, knn_file_name_prefix)
                for k in ks:
                    knn_query_output_path = QDTREE_KNN_QUERY_OUTPUT_PATH_3D.format(
                        data_file_prefix=data_file_prefix,
                        range_query_prefix=file_name_prefix,
                        knn_query_prefix=knn_file_name_prefix,
                        episode=episode,
                    ...
                    execute_knn_query(k, query_file, data_file, knn_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="qdtree")

            for file_name in point_queries:
                point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, point_file_name_prefix)
                point_query_output_path = QDTREE_POINT_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    point_query_prefix=point_file_name_prefix,
                    episode=episode,
                ...
                execute_point_query(query_file, data_file, point_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="qdtree")

            for file_name in spatial_joins:
                join_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, join_file_name_prefix)
                join_query_output_path = QDTREE_JOIN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    join_query_prefix=join_file_name_prefix,
                    episode=episode,
                ...
                execute_spatial_join(query_file, data_file, join_query_output_path, "test-kdtree-KDTreeQuery3d", index_name="qdtree")

            for file_name in insertions:
                insert_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, insert_file_name_prefix)
                insert_output_path = QDTREE_INSERT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    insert_prefix=insert_file_name_prefix,
                    episode=episode,
                ...
                execute_insert(query_file, insert_output_path, "test-kdtree-KDTreeQuery3d", index_name="qdtree")

            for file_name in insert_points:
                insert_point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, insert_point_file_name_prefix)
                insert_point_output_path = QDTREE_INSERT_POINT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    insert_point_prefix=insert_point_file_name_prefix,
                    episode=episode,
                ...
                execute_insert_point(query_file, insert_point_output_path, "test-kdtree-KDTreeQuery3d", index_name="qdtree")
...
def run_platon(data_file_name, query_maps, ks_map, baseline_config):
    ...
    try:
        ...
        for range_file_name in range_queries:
            ...
            platon_output_default = PLATON_PARTITION_OUTPUT_3D.format(
                data_file_prefix=data_file_prefix,
                query=file_name_prefix
            )

            build_output_path = PLATON_BUILD_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                query=file_name_prefix,
            )
            ...
            data_adapter_command = f"python tools/libspatialindex_data_adapter.py --type data --dimensions 3 --input {ablosute_data_file_name} --output {PLATON_DATA_3D}"
            execute_command(data_adapter_command)

            # command = f"test-rtree-PlatonBulkLoad {PLATON_DATA} {INDEX_PATH}/platon {page_size} {fill_factor} {BLOCK_SIZE} {BUFFER}"
            command = f"test-rtree-PlatonBulkLoad3d {PLATON_DATA_3D} {INDEX_PATH}/platon {page_size} {fill_factor} tgs {platon_output_default}"
            result, elapsed_time_ns_build = execute_command_with_err(command)
            ...
            data_file = PLATON_DATA_3D

            query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, file_name_prefix)
            range_query_output_path = PLATON_RANGE_QUERY_OUTPUT_PATH_3D.format(
                data_file_prefix=data_file_prefix,
                range_query_prefix=file_name_prefix,
            )
            ...
            for knn_file_name in knn_queries:
                ...
                ks = ks_map.get(knn_file_name)
                for k in ks:
                    knn_query_output_path = PLATON_KNN_QUERY_OUTPUT_PATH_3D.format(
                        data_file_prefix=data_file_prefix,
                        range_query_prefix=file_name_prefix,
                        knn_query_prefix=knn_file_name_prefix,
                        k=k
            ...
            for file_name in point_queries:
                point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, point_file_name_prefix)
                point_query_output_path = PLATON_POINT_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    point_query_prefix=point_file_name_prefix,
                )
            ...
            for file_name in spatial_joins:
                join_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, join_file_name_prefix)
                join_query_output_path = PLATON_JOIN_QUERY_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    join_query_prefix=join_file_name_prefix,
                )
                execute_spatial_join(query_file, data_file, join_query_output_path, index_name="platon")

            for file_name in insertions:
                insert_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, insert_file_name_prefix)
                insert_output_path = PLATON_INSERT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    insert_prefix=insert_file_name_prefix,
                )
            ...
            for file_name in insert_points:
                insert_point_file_name_prefix = file_name.rstrip('.csv')
                query_file = os.path.join(BENCHMARK_LIBSPATIALINDEX, insert_point_file_name_prefix)
                insert_point_output_path = PLATON_INSERT_POINT_OUTPUT_PATH_3D.format(
                    data_file_prefix=data_file_prefix,
                    range_query_prefix=file_name_prefix,
                    insert_point_prefix=insert_point_file_name_prefix,
                )
...
def process_experiment(experiment):
    ...
    if data_distribution in ["uniform", "normal", "skewed"]:
        ...
        data_file_name = RELATIVE_REAL_DATA_FILENAME_3D.format(
        ...
        if len(range_queries) == 0:
            ...
            if not os.path.exists(os.path.join(REAL_QUERY_PATH, REAL_RANGE_QUERY_FILENAME_DEFAULT_3D.format(data=base_name))):
