...
def execute_command_with_err(command):
    ...
    try:
        ...
        logger.error(f"Command failed: {e}")
        logger.error(f"Error info: {retult}")
        retult = None
...
def main():
    ...
    configs = ["example_config_debug_rlrtree.json"]
    # else:
...
# test-kdtree-KDTreeQuery benchmark/libspatialindex/us_10000_insert_10000_2_uniform_1 ./benchmark/tree intersection 0 # candidates = ["write_only", "balance_only", "write_heavy_only", "read_heavy_only"]
        # # candidates = ["write_only", "balance_only", "write_heavy_only"]
        # for root, dirs, files in os.walk(directory):
        #     if root.split("/")[-1] not in candidates:
        #         continue
