...
class SplitLearner:
        ...
        def Test10(self):
                self.network.load_state_dict(torch.load("./model/"+self.config.model_name+".pth"))
                self.network.eval()
                node_access = 0
                query_num = 0
                for i in range(10):
        ...
        def Train4(self):
                ...
                for epoch in range(self.config.epoch):
                        ...
                        fin.close()
                end_time = time.time()
                torch.save(self.network.state_dict(), "./model/"+self.config.model_name+'.'+self.id+'.pth')
                reward_log.close()
                loss_log.close()
        ...
        def Train3(self):
                ...
                for epoch in range(self.config.epoch):
                        ...
                        fin.close()
                end_time = time.time()
                torch.save(self.network.state_dict(), "./model/"+self.config.model_name+'.pth')
...
if __name__ == '__main__':
        ...
        if args.process == "training":
                            ...
                            if (i+1) > args.max_entry and (i+1) % rf == 0:
                                    ...
                                    for k in range(rf):
                                        ...
                                        #     query_rectangle = [x_center - x_length/2, x_center + x_length/2, y_center - y_length/2, y_center + y_length/2]
                                            query_rectangle = random.choice(query_rectangles)
            ...
            example = torch.rand(1, 4 * brain_final.action_space_size).to(brain_final.Q_eval.device)
...
#         brain_final.Q_eval.load_state_dict(torch.load(str(args.model_number)+'_insertion_'+str(args.data_distribution)+'_'+str(args.reference_tree_type)+'_k'+str(args.action_space_size)+'.pth'))
#         # brain_final.Q_eval.load_state_dict(torch.load(str(args.model_number)+'_insertion_'+str(args.data_distribution)+'_'+'rtree'+'_k'+str(args.action_space_size)+'.pth'))
#         rl_count = 0
#         non_rl_count = 0
