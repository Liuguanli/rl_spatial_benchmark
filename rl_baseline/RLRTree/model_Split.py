...
class SplitLearner:
    ...
    def Test3(self):
        self.network.load_state_dict(torch.load(str(self.config.data_distribution)+'_'+'k'+str(self.config.action_space)+'_testing'+'.pth'))
        # self.network.eval()
        # self.ResetObjLoader()
        self.tree.Clear()
...
#                print('{} nodes are added into the tree'.format(int(object_num * ratio_for_tree_construction)))
        ...
        end_time = time.time()
        # torch.save(self.network.state_dict(), 'split.pth')
        example = torch.rand(1, 4 * self.config.action_space).to(self.network.device)
...
if __name__ == '__main__':
    ...
    if args.action == 'train':
        ...
        spl_learner.training_dataset = model_dataset
